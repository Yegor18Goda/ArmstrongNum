...
public class ArmstrongNumber {
    ...
    private static void generateNums(int[] arr, long N) {
        ...
        if (arr.length > 1) { // если массив представляет двухзначное(и более) число
            do { // делаем пока последняя цифра больше нуля (условие в конце цикла)
                if (arr[last] >= arr[last - 1]) { // если последняя цифра больше предпоследней
                    if (arr[last] == 0 & arr[0] == 0) break; // это костыль
                    checkAddToSet(arr, N); // отсылаем на проверку
                    arr[last]--; // декрементируем последнюю
                }
                // заходим в цикл и строим новое число по принципу:
                // если любая предыдущая цифра больше посдедующей,
                // то декрементируем ее, а последующую заполняем девяткой
        ...
        } else { // если число состоит из одной цифры, то заполняем сет от этого числа до единицы
